...
- `outfits` table: id, name, vibe (office/evening/weekend/summer/autumn/street), created_at
- `outfit_items` join table: outfit_id, item_id, position
- Types already defined in `src/lib/types.ts` (Category, OutfitVibe, ClothingItem, Outfit) — reuse them, don't redefine
- Existing display components (`OutfitCard`, `OutfitDetailPanel`, `OutfitGrid`) already render real cutout photos per item and vibe-keyed scene backdrops (`SceneBackdrop.tsx`) — new outfits just need to exist as real rows; no new outfit-rendering UI is needed
